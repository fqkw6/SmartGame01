...
public sealed class ObjectPool : MonoSingleton<ObjectPool>
{
        ...
        if (prefab != null && !Instance.pooledObjects.ContainsKey(prefab))
        {
            ...
            if (initialPoolSize > 0)
            {
                ...
                while (list.Count < initialPoolSize)
                {
                    ...
                    obj.name = obj.name.Substring(0, obj.name.LastIndexOf("(Clone)"));
                    obj.transform.SetParent(parent, false);
                    list.Add(obj);
                }
                prefab.SetActive(active);
                //Instance.pooledTimer[prefab] = PooledTime;
            }
        }
    }
    ...
    public static void Recycle(GameObject obj)
    {
        ...
        {
            Debug.LogWarning("{0} has been recycled or not in pooled, it will be destroyed", obj);
            Destroy(obj);
        }
    }
